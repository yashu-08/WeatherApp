...
async function fetchsearchweather(loc){
    ...
    try{
        ...
        const data= await response.json();
        if (data.cod !== 200) {
            err.classList.add("active");  // Show error message
            err.innerText = "City not found! Please enter a valid city name.";
        } else {
            err.classList.remove("active"); // Hide error message if successful
        }
